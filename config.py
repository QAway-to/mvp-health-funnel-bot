...
class Config:
    ...
    # Общий секрет с LavaTop: им подписан вебхук об оплате картой. Пусто —
    # маршрут выключен целиком. Открытая точка выдачи доступа означала бы,
    # что премиум выпишет себе кто угодно, зная адрес.
    #
    # Своего значения нет — берём ключ API (`LAVA_API`). Одно значение вместо
    # двух: та же строка вписывается в поле вебхука в кабинете, и заводить
    # вторую переменную не нужно.
    #
    # Плата за это названа прямо: ключ API теперь ездит ещё и в заголовках
    # входящих уведомлений, поэтому его утечка означает не только доступ к
    # кассе, но и возможность подделать оплату. Решение заказчика, принято
    # осознанно. Отдельный секрет включается в любой момент — достаточно
    # задать LAVATOP_SECRET, он перебивает.
    LAVATOP_SECRET: Optional[str] = os.getenv("LAVATOP_SECRET") or os.getenv(
        "LAVA_API"
    ) or os.getenv("LAVATOP_API_KEY")
