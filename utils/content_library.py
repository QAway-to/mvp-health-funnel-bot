...
_TAG_SYNONYMS: dict[str, tuple[str, ...]] = {
    ...
    # Направления, открытые в августе 2026. Роликов по ним ещё нет — теги
    # заведены заранее, чтобы первый же пост в канал подбирался сам, а не
    # ждал правки кода. По закаливанию ролики уже сняты автором: четыре
    # видео «Шаг 1–4», и тег для них есть выше.
    "сон": (
        "сон", "сна", "спать", "спит", "засыпа", "заснуть", "уснуть",
        "бессонниц", "высыпа", "выспа", "недосып", "мелатонин", "будильник",
    ),
    "зарядка": ("зарядк", "разминк", "растяжк", "подъём", "подъем", "утренн"),
    "самомассаж": ("самомассаж", "размять", "разминать шею", "затекл", "зажат"),
    "массаж": ("массаж", "тайск", "мануальн", "поглажива", "разминани", "растирани"),
    "привычки": ("привычк", "зависимост", "бросить", "срыв", "триггер", "сила воли"),
    "алкоголь": ("алкогол", "пиво", "вино", "выпива", "спиртн", "похмель", "бухать"),
    "сахар": ("сахар", "сладк", "конфет", "торт", "десерт", "мучн", "хлеб", "выпечк"),
    "кофе": ("кофе", "кофеин", "чай", "энергетик"),
    "фастфуд": ("фастфуд", "фаст-фуд", "бургер", "чипс", "гмо", "мясо", "мяса"),
    "лень": ("лень", "лениво", "прокрастин", "мотивац"),
}
