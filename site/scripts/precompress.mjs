 ...
 *
 * Нужно варианту с Web Service: сжатием там никто не занимается, а sirv
 * отдаёт готовые архивы, если они лежат рядом. Static Site сжимает на CDN
 * сам — для него это лишние ~80 КБ в загрузке.
 *
 * Шаг вшит в общий `npm run build`, а не вынесен в отдельную команду:
 * тогда обе схемы деплоя используют одну build-команду и её нельзя
 * перепутать в настройках сервиса.
 */
