...
# --- личный бот заметок ------------------------------------------------------
#
# Модуль чужой этому проекту: свой токен, свои переменные, свой репозиторий, и
# ни одного импорта в обе стороны. Здесь — единственная точка соприкосновения.
#
# Стоит перед раздачей сайта намеренно: маршрут-ловушка ниже забирает всё, что
# не перехвачено раньше, и подключённый после неё вебхук просто не получал бы
# сообщений.
#
# try — не перестраховка. Личные заметки не имеют права помешать сервису,
# который принимает деньги: не собрался модуль — не подключился, остальное
# работает как работало.
try:
    from notes import is_configured as _notes_configured, router as _notes_router

    if _notes_configured():
        app.include_router(_notes_router)
        log_agent_action("App", "🗒 Бот заметок подключён")
except Exception as _notes_error:  # noqa: BLE001
    log_agent_action("App", f"Бот заметок не подключён: {_notes_error}", level="WARNING")
